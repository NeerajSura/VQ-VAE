...
class MnistLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, codebook_size):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(in_features=hidden_size // 4, out_features=codebook_size)
        )
        # Add padding and start token to embedding size
        self.word_embedding = nn.Embedding(codebook_size + 2, embedding_dim=input_size)
        ...
        for encoding_idx in tqdm(range(num_encodings)):
            ...
            for context, target in sents:
                ...
                if len(context) < self.context_size:
                    context = torch.nn.functional.pad(context, (0, self.context_size - len(context)), "constant",
                                                      self.pad_token)
