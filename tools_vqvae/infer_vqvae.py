...
def reconstruct(config, model, dataset, num_images = 100):
    ...
    # Dataset generates ims from -1 to 1, we convert to 0 to 1
    ims = (ims + 1)/2

    # Flip the generated images for better visualition, only if they are not colored
...
def inference(args):
    ...
    mnist_test = MnistDataset('test', config['train_params']['train_path'],
                              im_channels=config['model_params']['in_channels'])
    mnist_train_loader = DataLoader(mnist, batch_size=config['train_params']['batch_size'], shuffle=False,
                                    num_workers=0)
    with torch.no_grad():
